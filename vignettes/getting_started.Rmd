...
plot_expression('E15', genes = genes)
```
...
hippo_markers <- marker_df %>% 
    ...
    top_n(2, auc)
plot_expression('E15', genes=hippo_markers$gene, nrow=1)
```
...
We can further explore out voxel map more spatially. The `plot_map()` function lets us plot spatial similarity maps in a number of different ways. Per default, it will show us a sagittal view of the projection for each of the groups:

```{r, fig.height=2, fig.width=8}
plot_map(vox_map, nrow=1)
```
...
plot_map(vox_map, view='slice', slices=seq(1, 40, 4))
```
...
And plot the similarity to structure groups:

```{r, fig.height=4, fig.width=4}
plot_map(ref_map, annotation_level = 'structure_group')
```

We can again observe the patterns we've seen before: Cortical neurons are highly correlated with the neocortex (NCx), diencephalic neurons with the dorsal thalamus (DTH) and GE neurons with the ganglionic eminences (GE) and the amygdala (AMY).
...
```{r, eval=F, echo=T}
mb_map <- mousebrain_map(example_seurat, group_name = 'cluster', genes_use = top10)
```
...
Note: In this case, we only compute the similarity to aggregated clusters of the data, as correlation to >300.000 cells is computationally quite intense. Further the plotting function `plot_map()` takes an argument subsample, which allows downsampling the cells for plotting.
